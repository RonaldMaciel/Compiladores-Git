...
declarat : declarations declarat           { $$ = astCreate(AST_DECLARAT, 0, $1, $2, 0, 0, getLineNumber());}
         |                                 { $$ = 0;}
         ;
...
func_parameters : tipo TK_IDENTIFIER end_parameters        { $$ = astCreate(AST_PARAMINIT, $2, $1, $3, 0, 0, getLineNumber());}      
                |                                          { $$ = 0;} // pode ser vazio inte main()
                ;

end_parameters : ',' tipo TK_IDENTIFIER end_parameters     { $$ = astCreate(AST_PARAMLIST, $3, $2, $4, 0, 0, getLineNumber());}       
               |                                           { $$ = 0;} // pode ser vazio inte main()
               ;


literal : LIT_INT              { $$ = astCreate(AST_SYMBOL, $1, 0, 0, 0, 0, getLineNumber());}
        | LIT_CHAR             { $$ = astCreate(AST_SYMBOL, $1, 0, 0, 0, 0, getLineNumber());}
        | LIT_REAL             { $$ = astCreate(AST_SYMBOL, $1, 0, 0, 0, 0, getLineNumber());}
        ; 


tipo : KW_CHAR                 { $$ = astCreate(AST_DECCHAR, 0, 0, 0, 0, 0, getLineNumber());}
     | KW_INT                  { $$ = astCreate(AST_DECINT, 0, 0, 0, 0, 0, getLineNumber());}
     | KW_REAL                 { $$ = astCreate(AST_DECREAL, 0, 0, 0, 0, 0, getLineNumber());}
     | KW_BOOL                 { $$ = astCreate(AST_DECBOOL, 0, 0, 0, 0, 0, getLineNumber());}
     ;
...
bloco : '{' commands_list '}'             { $$ = astCreate(AST_BLOCO, 0, $2, 0, 0, 0, getLineNumber());}
      ;


commands_list : commands commands_list   { $$ = astCreate(AST_COMMANDOLIST, 0, $1, $2, 0, 0, getLineNumber());}
              |                          { $$ = 0; }
              ;
...
commands : bloco                                                { $$ = $1; }
         ...
         | KW_OUTPUT output_list ';'                            { $$ = astCreate(AST_OUTPUT, 0, $2, 0, 0, 0, getLineNumber());}
         | KW_RETURN expr ';'                                   { $$ = astCreate(AST_RETURN, 0, $2, 0, 0, 0, getLineNumber());}   
         | ';'                                                  { $$ = 0; }
         ;

output_list : elements_to_output end_output           { $$ = astCreate(AST_OUTPUTINIT, 0, $1, $2, 0, 0, getLineNumber());}
            ;
...
expr : literal                                  { $$ = $1; }
     ...
     | '(' expr ')'                             { $$ = astCreate(AST_PARENTESES, 0, $2, 0, 0, 0, getLineNumber());}    
     ;


arguments_list : expr arguments_list_end       { $$ = astCreate(AST_ARGFUNCCALL, 0, $1, $2, 0, 0, getLineNumber());}
               |                               { $$ = 0; }
               ;

arguments_list_end : ',' expr arguments_list_end    { $$ = astCreate(AST_ARGFUNCCALLLIST, 0, $2, $3, 0, 0, getLineNumber());}
                   |                                { $$ = 0; }
                   ;
